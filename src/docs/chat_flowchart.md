...
flowchart TD
    ...
    subgraph FetchFlow ["メッセージ取得フロー (fetchMessages)"]
        IsLoading{"通信中?<br>(isLoading)"}
        SetLoading["isLoading = true"]
        ApiGet["APIリクエスト<br>(GET /api/v1/items/:id/chats)"]
        Success{"成功?"}
    ...
    subgraph UserAction ["ユーザーアクション"]
        ...
        ApiPost["APIリクエスト<br>(POST /api/v1/items/:id/chats)"]
        PostSuccess{"成功?"}
        ResetForm["フォームリセット"]
        ClearMsg["メッセージリスト消去"]
        ...
        ApiDelete["APIリクエスト<br>(DELETE /api/v1/chats/:id)"]
        RemoveDOM["DOMから削除"]

        EventUpdate["編集保存 (submit)"]
        ApiPatch["APIリクエスト<br>(PATCH /api/v1/chats/:id)"]
    ...
    CheckElements -- あり --> InitVars
    InitVars --> CallFetchInit
    CallFetchInit --> IsLoading

    IsLoading -- Yes --> EndFetch
    IsLoading -- No --> SetLoading
    SetLoading --> ApiGet
    ApiGet --> Success
    Success -- No --> ErrorGet --> EndFetch
    Success -- Yes --> ParseData
    ParseData --> HasMessages
